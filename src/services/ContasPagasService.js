            ...
            const query = `
                 SELECT 
                    ...
                    CASE 
                        ...
                        WHEN fi.funcionario_id IS NOT NULL THEN (SELECT nome FROM dbgerencialdionegoulart.clientes WHERE id = fu.cliente_id)
                        WHEN fi.credor_nome is not null then fi.credor_nome
                    END AS credor_nome,
                    cb.nome AS conta_bancaria_nome
                FROM 
                    dbgerencialdionegoulart.financeiro fi
                    LEFT JOIN dbgerencialdionegoulart.fornecedores fo ON fo.id = fi.fornecedor_id
                    LEFT JOIN dbgerencialdionegoulart.clientes cl ON cl.id = fi.cliente_id
                    LEFT JOIN dbgerencialdionegoulart.funcionarios fu ON fu.id = fi.funcionario_id
                    INNER JOIN dbgerencialdionegoulart.movimentacaofinanceira mf ON (mf.financeiro_id = fi.id and fi.status <> 'cancelada')
                    INNER JOIN dbgerencialdionegoulart.contasbancarias cb ON cb.id = mf.conta_id
